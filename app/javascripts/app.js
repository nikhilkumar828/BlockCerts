...
import bc_artifacts from '../../build/contracts/blockCerts.json'

// Conference is our usable abstraction, which we'll use through the code below.
var BC = contract(bc_artifacts);
...
$("#tentantAddress").text(web3.eth.accounts[0]); //prints account address
    ...
    initializeConference: function() {
        ...
        BC.deployed().then(function(instance) {
            sim = instance;
...
giveCert: function(pr,ti,ra) 
{
        var simp;
        BC.deployed().

	then( function(instance) { simp = instance; simp.giveCert(pr,ti,url,{from: accounts[0],gas:3000000}). //(}
...
getCerts: function(pr) 
{
        var simp;
        BC.deployed().

	then( function(instance) { simp = instance; simp.getCerts(pr,0,{from: accounts[0],gas:3000000}). //(}


	then( function(ipfs) { console.log(ipfs); var msgResult="Finally";  $("#disp").html(msgResult); });  }). 
        catch(function(e) { console.log(e); });
...
window.addEventListener('load', function() {
    ...
    BC.setProvider(web3.currentProvider);
    App.start();
